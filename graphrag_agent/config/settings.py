...
# ===== Legal GraphRAG / 本地抽取 / 训练配置 =====

ENABLE_LEGAL_DOMAIN = _get_env_bool("ENABLE_LEGAL_DOMAIN", False)
LEGAL_DOMAIN_DIR = Path(
    os.getenv("LEGAL_DOMAIN_DIR", PROJECT_ROOT / "domains" / "legal")
).expanduser()

ENABLE_LOCAL_EXTRACTOR = _get_env_bool("ENABLE_LOCAL_EXTRACTOR", False)
...
LOCAL_EXTRACTOR_MAX_NEW_TOKENS = (
    _get_env_int("LOCAL_EXTRACTOR_MAX_NEW_TOKENS", 512) or 512
)
LOCAL_EXTRACTOR_USE_MOCK = _get_env_bool("LOCAL_EXTRACTOR_USE_MOCK", True)

ENABLE_EXTRACT_FALLBACK = _get_env_bool("ENABLE_EXTRACT_FALLBACK", True)
LEGAL_EMBEDDING_MODEL_NAME = os.getenv(
